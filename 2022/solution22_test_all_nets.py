...
def follow_commands(z,dz,commands,data):
    ...
    return z,dz,commands

def find_connections(index,coords,padded_data,connections,border,rem_sides_nbs):
    cut_corners = 0
    while True:
        for (yc,xc),(dy0,dx0) in coords.copy():
            coords.pop(0)
            if (yc,xc) not in border:
                continue
            padded_data[yc,xc] = str(chr(ord("A")+index))
            connections.setdefault(index,[]).append((index,yc,xc,dy0,dx0))
            border.remove((yc,xc))
            nbs=[(c,(dy,dx)) for dy,dx in [(0,1),(0,-1),(1,0),(-1,0)] if (c:=(yc+dy,xc+dx)) in border]
            if not nbs:
                cut_corners+=1
                nbs=[(c,(dy,dx)) for dy,dx in product(*[(-1,0,1)]*2) if (c:=(yc+dy,xc+dx)) in border]
            if nbs:
                coords.extend(nbs)
                rem_sides_nbs.setdefault(index-1,[]).extend(nbs)
        index+=1
        if not coords:
            rem_sides_nbs.setdefault(index-1,[]).extend(nbs)
            if cut_corners==2:
...
def move_and_act(z,dz,turn,walk,parity,*,at_turn=lambda :None,at_walk=lambda :None,at_corner=lambda :None,args=()):
    if turn:
        z += (1+parity*1j)*dz
        dz *= parity*1j
        at_turn(*args)
